fun countConsecutiveChars(str: String): String {
    ...
    for (i in 1 until str.length) {
        if (str[i] == currentChar && currentChar != ' ') {
            ...
            counterCharInfo(result, currentChar, count)
            currentChar = str[i]
            count = 1
        }
    }
...
private fun counterCharInfo(builder: StringBuilder, char: Char, count: Int) {
    builder.append(char)
    if (count > 1 && char != ' ') {
...
fun main() {
    println("Введите сообщение, в котором некоторые буквы подряд повторяются, например: \"aabbccddd\". Обрати внимание, что ПРОБЕЛЫ НЕ СЖИМАЮТСЯ. Приятного пользования ;)\n")
    while (true) {
        try {
